...
function DealsTable({ selectedOwner, selectedStage, createdDate, closedDate, activePage, pageSize, onFilteredCount }) {
    ...
    const dealsArray = Array.isArray(deals) ? deals : Array.isArray(deals?.data) ? deals.data : [];
    ...
    const filteredDeals = dealsArray.filter((deal) => {
        ...
        if (selectedOwner || selectedOwner != "All") {
            ...
            match = match && (deal.owner?.userId === selectedOwner || deal.dealOwner === selectedOwner);
        }
